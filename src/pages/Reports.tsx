  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl">
      ...
      <Tabs defaultValue="today" className="w-full">
        ...
        <TabsContent value="range" className="mt-6 bg-white p-6 rounded-2xl border border-muted premium-shadow animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="flex flex-wrap gap-4 items-end">
            ...
            <Button onClick={fetchDateRange} disabled={loading} className="h-10 px-8 rounded-xl premium-shadow font-bold uppercase tracking-widest text-xs">
              Generate Report
            </Button>
          </div>
        </TabsContent>
      ...
      {!loading && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="border-none premium-shadow bg-white rounded-2xl premium-hover overflow-hidden transition-all duration-300">
              <CardContent className="p-6">
                <div className="flex items-center gap-4">
                  <div className="bg-primary/10 rounded-2xl p-4 shrink-0"><Receipt className="w-6 h-6 text-primary" /></div>
                  <div>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-none mb-1">Total Orders</p>
                    <p className="text-3xl font-black tracking-tighter tabular-nums">{totalOrders}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            <Card className="border-none premium-shadow bg-white rounded-2xl premium-hover overflow-hidden transition-all duration-300">
              <CardContent className="p-6">
                <div className="flex items-center gap-4">
                  <div className="bg-accent/10 rounded-2xl p-4 shrink-0"><DollarSign className="w-6 h-6 text-accent" /></div>
                  <div>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-none mb-1">Total Revenue</p>
                    <p className="text-3xl font-black tracking-tighter tabular-nums text-accent">Rs. {totalRevenue.toLocaleString()}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            <Card className="border-none premium-shadow bg-white rounded-2xl premium-hover overflow-hidden transition-all duration-300">
              <CardContent className="p-6">
                <div className="flex items-center gap-4">
                  <div className="bg-secondary/30 rounded-2xl p-4 shrink-0"><TrendingUp className="w-6 h-6 text-secondary-foreground" /></div>
                  <div>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-none mb-1">Avg Ticket</p>
                    <p className="text-3xl font-black tracking-tighter tabular-nums text-secondary-foreground">Rs. {Math.round(avgOrder).toLocaleString()}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
